...
Apply the Calico CNI plugin as a network overlay.
In the Controller server terminal, run the following command to apply Calico:
...
Execute a command directly on a pod to verify NGINX version
...
Curl the port on that node

curl -I <node-dns-name>:<node-port>
We should see a status of 200 OK.
